package rabbitmq.streaming

import java.nio.ByteBuffer

object UnsubscribeCodec {
  def encode(request: UnsubscribeRequest, correlationId: Int): ByteBuffer = {
    val totalSize = Protocol.Sizes.Key +
      Protocol.Sizes.Version +
      Protocol.Sizes.CorrelationId +
      Protocol.Sizes.SubscriptionId

    val buffer = Protocol.allocate(totalSize)

    buffer.putShort(Protocol.Commands.Unsubscribe)
    buffer.putShort(Protocol.ProtocolVersion)
    buffer.putInt(correlationId)
    buffer.put(request.subscriptionId)

    buffer
  }

  def decode(
      buffer: ByteBuffer,
      expectedKey: Short,
      version: Short
  ): Either[String, UnsubscribeResponse] = {
    for {
      key <- Either.cond(
        expectedKey == Protocol.Commands.UnsubscribeResponse,
        (),
        s"Invalid key field"
      )
      correlationId = buffer.getInt()
      responseCode = buffer.getShort()
    } yield UnsubscribeResponse(correlationId, responseCode)
  }
}