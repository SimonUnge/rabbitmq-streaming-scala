...
object ConnectionActor {
  ...
  case class SendUnsubscribe(request: UnsubscribeRequest)
      extends Command // Fire-and-forget
  case class SendCredit(request: CreditRequest) extends Command
  case class SendDeletePublisher(request: DeletePublisherRequest)
      extends Command
    ...
    Behaviors
      .receive[Command] { (context, message) =>
        message match {
          ...
          case SendDeletePublisher(request) =>
            val nextId = correlationId + 1
            val buffer = DeletePublisherCodec.encode(request, nextId)
            connection.sendFrame(buffer)
            connected(
              connection,
              nextId,
              pending,
              publishers - request.publisherId,
              subscribers
            )
